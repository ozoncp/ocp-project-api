...
var (
	// we really need this mutex for correct processing signals
...
func runGrpcAndGateway() error {
	...
	projectStorage := storage.NewProjectStorage(db, config.Global.DB.ChunkSize)

	GrpcMutex.Lock()
	GrpcServer = grpc.NewServer()
	// reflection is needed for grpcui
	...
	group.Go(func() error {
		err = desc.RegisterOcpProjectApiHandlerFromEndpoint(
			ctx,
			gwmux,
			config.Global.Service.GrpcEndpoint(), opts)
		...
		log.Info().Msgf("Grpc gateway server listening on %s", config.Global.Service.HttpEndpoint())
